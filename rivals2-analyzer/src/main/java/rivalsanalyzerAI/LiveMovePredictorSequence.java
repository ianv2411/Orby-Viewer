...
public class LiveMovePredictorSequence {
    ...
    static Map<String, Integer> moveCounts = new HashMap<>();

    static LinkedList<INDArray> predictionHistory = new LinkedList<>();
    static int SMOOTH_WINDOW = 12;

    static String lastMove = "";
    static String pendingMove = "";
    static int pendingCount = 0;
    static final int STABILITY_FRAMES = 2;
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (true) {
            ...
            if (boxToUse.x() < 0 || boxToUse.y() < 0) continue;
            if (boxToUse.width() <= 0 || boxToUse.height() <= 0) continue;
            if (boxToUse.x() + boxToUse.width() > screen.cols()) continue;
            if (boxToUse.y() + boxToUse.height() > screen.rows()) continue;
    ...
    public static Rectangle getGameWindowBounds() {
        ...
        if (rect[0] == null) {
            throw new RuntimeException("Game window not found");
        }
    ...
    public static Rect detectZetterburn(Mat screen, Mat template) {
        ...
        int w = Math.min(template.cols() + 200, screen.cols() - x);
        int h = Math.min(template.rows() + 120, screen.rows() - y);
